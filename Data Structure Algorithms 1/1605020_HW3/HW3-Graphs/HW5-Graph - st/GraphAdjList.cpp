...
void Graph::removeEdge(int u, int v)
{
    ...
    if(t != NULL_VALUE)
    {
        adjList[u].removeItemAt(t);
        this->nEdges--;
    }
    if(!directed)
    {
        t = adjList[v].searchItem(u);
        adjList[v].removeItemAt(t);
    }
}
...
bool Graph::hasCommonAdjacent(int u, int v)
{
    ...
    {
        int t = adjList[u].getItem(i);

        for(int j=0;j<adjList[v].getLength();j++)
        {
             int s = adjList[v].searchItem(t);

             if(s != NULL_VALUE) return true;
        }
    }
